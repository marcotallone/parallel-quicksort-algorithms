...
def preproc(filename):
    ...
    # Find the serial time for each value of "n" and its standard deviation
    serial_time = {}
    serial_std = {}
    for n in ns:
        serial_time[n] = df[(df["p"] == 1) & (df["n"] == n)]["t"].mean()
        serial_std[n] = df[(df["p"] == 1) & (df["n"] == n)]["std"].mean()

    # Calculate the speedup and efficiency for each value of "n" (and stds)
    for n in ns:
        df.loc[df["n"] == n, "sp"] = serial_time[n] / df[df["n"] == n]["t"]
        df.loc[(df["n"] / df["p"]) == n, "eff"] = serial_time[n] / df[(df["n"] / df["p"]) == n]["t"]

        df.loc[df["n"] == n, "sp_std"] = ( (serial_std[n]**2 / df[df["n"] == n]["t"]**2)
                                          + (df[df["n"] == n]["std"]**2 * serial_time[n]**2 / df[df["n"] == n]["t"]**4) ).apply(np.sqrt)

        df.loc[(df["n"] / df["p"]) == n, "eff_std"] = ( (serial_std[n]**2 / df[(df["n"] / df["p"]) == n]["t"]**2)
                                            + (df[(df["n"] / df["p"]) == n]["std"]**2 * serial_time[n]**2 / df[(df["n"] / df["p"]) == n]["t"]**4) ).apply(np.sqrt)
    return df